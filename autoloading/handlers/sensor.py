...
def read_per_second(loadpoint: LoadPoint):  # 每秒读取一次物位
    ...
    # 发送物位计数据到前端
    sensor_data({"value": int_distance, "tablename": loadpoint.Sensor.__tablename__})
...
def insert_data(Sensor, int_distance, current_time):
    sensor = Sensor(
        data=int_distance, time=current_time, tablename=Sensor.__tablename__
    )
