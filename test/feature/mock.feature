Feature: connect接口测试(模拟装车模式)
  Scenario Outline: 5.1.1 1、集卡引导到位，获取时庐的PLC控制策略
    ...
    Examples:
      ...
      | ./test/json/postdata-1.json | [1.1]           | ./csv/sensor14-2.csv | 5.5          | [[1.1], []]                                 |
  ...
  Scenario Outline: 5.1.3 2、反馈出闸信息
    ...
    Examples:
      ...
      | ./test/json/postdata-1.json | [1.1]           | ./csv/sensor14-2.csv | ['闽1','闽2'] | [-1, 40]       | 0.05         | [[1.1], []]                              |
      | ./test/json/postdata-1.json | [1.2, 1.1]      | ./csv/sensor14-2.csv | ['闽1','闽2'] | [-1, 40]       | 0.05         | [[1.2, 1.1], [1.1], []]                  |
      | ./test/json/postdata-1.json | [1.1, 1.2]      | ./csv/sensor14-2.csv | ['闽1','闽2'] | [-1, 40]       | 0.05         | [[1.1, 1.2], [1.2], []]                  |
  ...
  Scenario Outline: 5.1.1 1、集卡引导到位，获取时庐的PLC控制策略，第一次装料物料就满了直接结束
    ...
    Examples:
      | file                        | distance_list   | csv_file             | load_current | expected_icps_differ_list     |
      | ./test/json/postdata-1.json | [1.1]           | ./csv/sensor14-2.csv | 0.0          | [[1.1], []]                   |
      | ./test/json/postdata-1.json | [1.2, 1.1]      | ./csv/sensor14-2.csv | 0.0          | [[1.2, 1.1], [], []]          |
      | ./test/json/postdata-1.json | [1.1, 1.2]      | ./csv/sensor14-2.csv | 0.0          | [[1.1, 1.2], [], []]          |
  ...
  #   And 模拟重量估计公式（高度）
    ...
    Examples:
      ...
      | ./test/json/postdata-1.json    | [1.1]           | ./csv/sensor14-3.csv    | 油菜籽        | 28.3         | [[1.1], []]                   |
      | ./test/json/postdata-601A.json | [1.2, 1.1]      | ./csv/sensor16-8-14.csv | 黄豆          | 28.54        | [[1.2, 1.1], [], []]          |
