Feature: connect接口测试
  Scenario Outline: 2.1.1 将任务信息传给时庐获取引导策略
    ...
    | file                        | guide_position                   | expected_icps_differ      |
    | ./test/json/postdata-1.json | [1.1]                            | [1.1]                     |
    | ./test/json/postdata-1.json | [1.2, 1.1]                       | [1.2, 1.1]                |
    | ./test/json/postdata-1.json | [1.1, 1.2, 1.3]                  | [1.1, 1.2, 1.3]           |
    | ./test/json/postdata-1.json | [1.1, 1.2, 1.3, 1.4]             | [1.1, 1.2, 1.3, 1.4]      |
  ...
  Scenario Outline: 2.1.2 集卡引导到位，获取时庐的 PLC 控制策略
    ...
    Examples: 
      | file                        | guide_position         | csv_file              | expected_return_file |
      | ./test/json/postdata-1.json | [1.1]                  | ./csv/sensor4-1-1.csv | ./test/ret/ret1.json |
      | ./test/json/postdata-2.json | [1.1]                  | ./csv/sensor4-1-1.csv | ./test/ret/ret2.json |
      | ./test/json/postdata-1.json | [1.1, 1.2]             | ./csv/sensor4-1-2.csv | ./test/ret/ret3.json |
  ...
  Scenario Outline: 2.1.2 集卡引导到位，获取时庐的 PLC 控制策略——异常测试(未请求data_type=0)
    ...
    Examples: 
      | file                        | guide_position | csv_file              | expected_height_load |
      | ./test/json/postdata-1.json | [1.1]          | ./csv/sensor4-1-1.csv | -1                   |
  ...
  Scenario Outline: 2.1.3 装车完成后，获取出闸重量
    ...
    Examples: 5种装车过程（最后一种为错误情况）
    | file                        | guide_position  | weight_out_list | expected_weight_out |
    | ./test/json/postdata-1.json | [1.1]           | [20]            | 20                  |
    | ./test/json/postdata-1.json | [1.1, 1.2]      | [20]            | 20                  |
    | ./test/json/postdata-1.json | [1.2, 1.1]      | [20]            | 20                  |
  ...
  Scenario Outline: 2.1.3 两辆车装车完成后，合作方提供第一辆的出闸重量
    ...
    Examples: 5种装车过程（最后一种为错误情况）
    | file                        | guide_position | plate_list            | weight_out_list | expected_weight_out |
    | ./test/json/postdata-1.json | [1.1]          | ["A86", "A87", "A86"] | [20]            | 20                  |
    | ./test/json/postdata-1.json | [1.1, 1.2]     | ["A86", "A87", "A86"] | [20]            | 20                  |
    | ./test/json/postdata-1.json | [1.1]          | ["A86", "B38", "A86"] | [20]            | 20                  |
