...
@given(parsers.parse("开口类型: {opening_type}"), target_fixture="opening_type")
def open_type(opening_type):
    # print(f'open_type: {open_type}')
    return opening_type
...
@given(
    parsers.parse("装料用时: {time_difference}"),
    target_fixture="time_difference",
    converters={"time_difference": float},
)
...
def call_weight_estimate(
    app, db, lpdict, goods_type, opening_type, loader_id, time_difference, csv_file
):
    with app.app_context():
        df = pd.read_csv(csv_file)
        df.to_sql(
            con=db.engine, name=Traffic.__tablename__, if_exists="append", index=False
        )
        estimated_weight = lpdict.get(loader_id).weight_estimate(
            goods_type, loader_id, opening_type, "h", 3.5, 0, 1.2
        )
