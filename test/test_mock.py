...
@then(
    parsers.parse("模拟请求，正常装车: {expected_icps_differ_list}"),
    converters={"expected_icps_differ_list": ast.literal_eval},
)
def check_strategy_1_normal(
    app,
    db,
    client,
    postdata,
    ...
    for _ in range(5):
        ...
        actual_icps_differ = (
            [actual_icps_differ]
            if type(actual_icps_differ) is not list
            else actual_icps_differ
        )
    ...
    postdata["operating_stations"]["icps_differ_current"] = ea_index
    ...
    for response in gen_post:
        ...
        # 点位移动测试
        if (
            responsedata.get("operating_stations").get("allow_plc_work") == 0
            and responsedata.get("operating_stations").get("work_finish") != 1
        ):
            ea_index += 1
            ...
            # postdata["operating_stations"]["icps_differ_current"] = ea_list[ea_index][0]
            postdata["operating_stations"]["icps_differ_current"] = ea_index
            actual_icps_differ = response0.json.get("operating_stations").get(
                "icps_differ"
            )
            actual_icps_differ = (
                [actual_icps_differ]
            ...
            assert (
                response0.json.get("operating_stations").get("icps_differ")
                == ea_list[ea_index][1]
            )
...
@then(
    parsers.parse("模拟请求，正常装车(车队): {expected_icps_differ_list}"),
    converters={"expected_icps_differ_list": ast.literal_eval},
)
def check_strategy_1_normal_carlist(
    app,
    db,
    client,
    postdata,
    ...
    for plate, weightout in pw_list:
        ...
        if weightout != -1:
            ...
            assert (
                response2.json.get("operating_stations").get("truck_id")
                == plate_list[0]
            )
            postdata["operating_stations"]["truck_id"] = plate
            with app.app_context():
                ...
                last_traffic = (
                    db.session.query(Traffic).filter_by(truckid=plate_list[0]).first()
                )
...
@then(
    parsers.parse("模拟请求，重量测试: {expected_weight_list}"),
    converters={"expected_weight_list": ast.literal_eval},
)
def check_strategy_1_normal_weight_estimate(
    app,
    db,
    client,
    postdata,
    ...
    postdata["data_type"] = 1
    postdata["operating_stations"]["icps_differ_current"] = icps_index
    ...
    for response in gen_post:
        ...
        # 打印重量测试
        weight_estimate_new = responsedata.get("operating_stations").get(
            "work_weight_reality"
        )
