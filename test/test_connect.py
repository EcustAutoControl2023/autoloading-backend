...
@given(
    parsers.parse("测试的post文件为: {file}"),
    target_fixture="jsondata",
    converters={"file": str},
)
def jsondata(file):
...
@given(parsers.parse("装料点位: {guide_position}"), converters={"guide_position": str})
def clientdata(jsondata, guide_position):
    # 将字符串转换为列表
    guide_position = json.loads(guide_position)
    printr(guide_position, "guide_position")

    jsondata["operating_stations"]["guide_position"] = guide_position
...
def post1(client, jsondata):
    ...
    for i in range(len(jsondata["operating_stations"]["guide_position"])):
        jsondata["operating_stations"]["icps_differ_current"] = i
        if i == len(jsondata["operating_stations"]["guide_position"]) - 1:
            jsondata["operating_stations"]["temp_manual_stop"] = jsondata[
                "temp_manual_stop"
            ]
...
@then(
    parsers.parse(
        "使用生成器模拟请求，返回正确的装车策略直至装车停止: {expected_return_file}"
    )
)
def check_strategy_1(app, db, gen_post, expected_return_file):
    ...
    for response in gen_post:
        with app.app_context():
            ...
            assert (
                "work_weight_reality" in responsedata.get("operating_stations").keys()
            )
...
@scenario(
    "./feature/connect.feature", "2.1.3 两辆车装车完成后，合作方提供第一辆的出闸重量"
)
def test_two_truck():
    pass
