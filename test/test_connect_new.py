...
def check_strategy_exception(app, client, db, postdata, distance_list):
    ...
    postdata["operating_stations"]["icps_differ_current"] = 0
    ...
    postdata["operating_stations"]["icps_differ_current"] = (
        0 if len(distance_list) > 0 else None
    )
...
@then(
    parsers.parse("测试装车策略（手动停止）: {expected_icps_differ_list}"),
    converters={"expected_icps_differ_list": ast.literal_eval},
)
def check_strategy_manualstop(
    app, client, db, expected_icps_differ_list, postdata, distance_list
):
    ...
    for distance, expected_icps_differ in ea_zip:
        ...
        # 点位移动
        postdata["data_type"] = 1
        postdata["operating_stations"]["icps_differ_current"] = distance_list.index(
            distance
        )
        response1 = client.post("/connect", json=postdata)
        check_response(response1, 1)
